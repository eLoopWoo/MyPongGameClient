...
bool Client::ProcessPacket(Packet _packettype)
{
	...
	case P_move:
	{
		...
		// [player1,player2,ball_x,ball_y,player1_score,player2_score]
		player_y1 = arr[0];
		player_y2 = arr[1];
		ball_x = arr[2];
		ball_y = arr[3];
		player1_score = arr[4];
		player2_score = arr[5];
		printf("| player_y1 = %F | | player_y2 = %F | | ball_x = %F | | ball_y = %F | | player1_score = %F || player2_score = %F | ", player_y1, player_y2, ball_x, ball_y, player1_score, player2_score);
		break;
	}
...
void Client::setIP(std::string IP,int PORT) {
