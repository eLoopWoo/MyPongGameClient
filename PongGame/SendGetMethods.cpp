...
bool Client::GetMove(float *_x) {
	printf("GetMoveFunc!");
	char * buffer = (char*)_x;
	if (!recvall(buffer, 24)) //receive message and store the message in buffer array. If buffer fails to be received...
		return false; //return false: Fails to receive 
...
bool Client::SendMoveUP1()
{
	if (!SendPacketType(P_moveUP1)) //Send packet type: Chat Message, If sending packet type fails...
		return false; //Return false: Failed to send string
	return true; //Return true: string successfully sent
}

bool Client::SendMoveDOWN1()
{
	if (!SendPacketType(P_moveDOWN1)) //Send packet type: Chat Message, If sending packet type fails...
		return false; //Return false: Failed to send string
	return true; //Return true: string successfully sent
}


bool Client::SendMoveUP2()
{
	if (!SendPacketType(P_moveUP2)) //Send packet type: Chat Message, If sending packet type fails...
		return false; //Return false: Failed to send string
	return true; //Return true: string successfully sent
}

bool Client::SendMoveDOWN2()
{
	if (!SendPacketType(P_moveDOWN2)) //Send packet type: Chat Message, If sending packet type fails...
		return false; //Return false: Failed to send string
	return true; //Return true: string successfully sent
}